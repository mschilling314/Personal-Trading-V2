...
def get_access_token():
    ...
    app_key = environ["Schwab_app_key"]
    app_secret = environ["Schwab_app_secret"]
    refresh_token = environ["Schwab_refresh_token"]
    headers = {'Authorization': f'Basic {b64encode(bytes(f"{app_key}:{app_secret}", "utf-8")).decode("utf-8")}',
               'Content-Type': 'application/x-www-form-urlencoded'}
    data = {'grant_type': 'refresh_token', 'refresh_token': refresh_token}
    resp = post('https://api.schwabapi.com/v1/oauth/token', headers=headers, data=data)
...
def get_refresh_token(callback_url: str="https://127.0.0.1"):
    ...
    headers = {'Authorization': f'Basic {b64encode(bytes(f"{app_key}:{app_secret}", "utf-8")).decode("utf-8")}', 'Content-Type': 'application/x-www-form-urlencoded'}
    data = {'grant_type': 'authorization_code', 'code': authorization_code, 'redirect_uri': callback_url}
    try:
        resp = post(auth_url, headers=headers, data=data)
        if resp.ok():
