...
def store_refresh_token(refresh_token):
    """
    Store the refresh token to a Dynamo table after encryption.  TODO: figure out how to run locally.
    """
    encrypted_refresh_token = encrypt(key_alias=os.environ["key_alias"], secret=refresh_token)
    store_secret(secret_name="refresh_token", secret_value=encrypted_refresh_token)


def get_access_token():
    """
    Assumes new access token is needed because of OCO idea to have fewer executions.
    """
    # TODO: add logic to get access token and check for expiration
    access_token = os.environ["schwab_access_token"]
    # TODO: Use refresh token to get and return OATH token, or return access token if it already exists


def _expired_huh(token) -> bool:
    pass
